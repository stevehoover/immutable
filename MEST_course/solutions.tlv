...
\m5
   ...
   // ---SETTINGS---
   universal_var(my_design, m5_if(m5_CalcLab, tt_um_calc, tt_um_riscv_cpu)) /// Change to tt_um_<your-github-username>_riscv_cpu. (See Tiny Tapeout repo README.md.)
   universal_var(debounce_inputs, 0)         /// Set to 1 to provide synchronization and debouncing on all input signals.
                                             /// use "m5_neq(m5_MAKERCHIP, 1)" to debounce unless in Makerchip.
   // --------------
   
   // If debouncing, a user's module is within a wrapper, so it has a different name.
   universal_var(user_module_name, m5_if(m5_debounce_inputs, my_design, m5_my_design))
...
module top(input logic clk, input logic reset, input logic [31:0] cyc_cnt, output logic passed, output logic failed);
   m5_if(m5_CalcLab, /* verilator lint_off WIDTH */)
